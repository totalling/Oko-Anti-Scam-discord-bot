...
const { getLogger, paint, colors } = require('./logger');
...
async function syncCommands(token, clientId, commandJson, { force = false } = {}) {
  ...
  const hashPreview = paint(colors.dim, hash.slice(0, 8));
  let previous = null;
  try {
  ...
  if (!force && previous === hash) {
    logger.info(
      `${paint(colors.yellow, 'skip')} ${commandJson.length} command(s) unchanged (hash ${hashPreview}), 0 API calls`
    );
    return false;
  }
  ...
  logger.info(
    `${paint(colors.green, 'synced')} ${commandJson.length} command(s) in ${Date.now() - startedAt}ms ` +
      `(1 bulk PUT, hash ${hashPreview})`
  );
  return true;
}
