...
cd oko-anti-crypto-scam-discord-bot/js
npm install
```
...
```
js/src/commands/    Slash commands & context menu (/scam, /invite, /support, /botinfo, Mark as Known Scam)
js/src/events/      Event listeners (message scanning, joins, guild welcome, interactions)
js/src/detection/   Scam scoring — heuristics, OCR, perceptual image hashing
js/src/moderation/  Punishment actions, per-guild settings, mod-log UI (Components V2)
js/src/deploy.js    Hash-based command sync — skips registration when commands are unchanged
